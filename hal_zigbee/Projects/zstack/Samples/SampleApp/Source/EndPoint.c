...
{ 
  ...
  // A message with a serial data block to be transmitted on the serial port.
    ...
    addr = pkt->cmd.Data[0];//获取地址是否为1号
    if(addr==1)
    {
      cmd=pkt->cmd.Data[1];//获取开关灯指令
      if(cmd =='A') //315电子 改动此处为21即使第二个终端的代码
      {
       
        fre=(pkt->cmd.Data[2])*1000;
       // HalLedSet( HAL_LED_3, HAL_LED_MODE_OFF);//对应的板子D1灯开
        HalLedBlink (HAL_LED_3, 0, 50,fre);//闪烁
      }
      if(cmd =='B')//315电子 改动此处为20即使第二个终端的代码
      {
       
        HalLedSet( HAL_LED_3, HAL_LED_MODE_ON);//对应的板子D1灯点关
      }
    }
}
/*********************************************************************
